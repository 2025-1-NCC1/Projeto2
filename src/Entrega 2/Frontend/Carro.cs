...
public class CarroMover : MonoBehaviour
{
    ...
    private float tempoDecorrido = 0f; // Tempo que passou desde a ultima mudança de estado
    private enum Estado { Indo, Espera1, Voltando, Espera2 } // Enumeração que define os estados poss�veis do carro: indo, esperando, voltando, esperando
    ...
    void Update()
    {
        ...
        switch (estadoAtual)
        {
            ...
            case Estado.Voltando:
                // Carro anda para frente novamente (que agora � "para tras" por causa da rotaçãoo)
